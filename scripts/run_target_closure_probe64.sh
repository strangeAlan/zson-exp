...
RUN_ID="${ZSON3_RUN_ID:-ofbase_target_closure_v0_probe64_seed${SEED}}"
...
printf '[ZSON3 LAUNCH] target_closure_v0 probe64 run_id=%s manifest_sha256=%s\n' \
...
"${PYTHON}" -u scripts/run_hm3dv1_random100.py \
  ...
  --max-steps 500 \
  --max-time 3600 \
  --config config/zson3/navigation_hm3dv2_target_closure.yaml \
  --output-dir "${OUT_DIR}" \
  --resume \
