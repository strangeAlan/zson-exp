...
RUN_ID="${ZSON3_RUN_ID:-ofbase_geometry_frontier_v1_probe56_seed${SEED}}"
...
printf '[ZSON3 LAUNCH] geometry_frontier_v1 probe run_id=%s probe_sha256=%s source_sha256=%s\n' \
...
"${PYTHON}" -u scripts/run_hm3dv1_random100.py \
  ...
  --config config/zson3/navigation_hm3dv2_geometry_frontier_v1.yaml \
  --output-dir "${OUT_DIR}" \
  --resume \
