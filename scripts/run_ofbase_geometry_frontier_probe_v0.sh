...
RUN_ID="${ZSON3_RUN_ID:-ofbase_geometry_frontier_v0_probe56_seed${SEED}}"
...
if [[ "${probe_sha256}" != "${EXPECTED_PROBE_SHA256}" ]]; then
  echo "Geometry probe manifest hash mismatch: ${probe_sha256}" >&2
  exit 1
fi
...
printf '[ZSON3 LAUNCH] geometry_frontier_v0 probe56 run_id=%s probe_sha256=%s source_sha256=%s\n' \
...
"${PYTHON}" -u scripts/run_hm3dv1_random100.py \
  ...
  --max-steps 500 \
  --max-time 3600 \
  --config config/zson3/navigation_hm3dv2_geometry_frontier_v0.yaml \
  --output-dir "${OUT_DIR}" \
  --resume \
  2>&1 | tee -a "${OUT_DIR}/raw.log" | stdbuf -oL grep --line-buffered -E \
    '^\[ZSON3 EVAL\]|^\[ZSON3 ABORT\]|^\[ZSON3 SUMMARY\]|Geometry completion|Traceback|Exception|CUDA out of memory|Killed' \
